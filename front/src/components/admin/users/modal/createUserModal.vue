<script>
    ...
    export default {
        ...
        computed: {
            ...
            oauth2() {return process.env.VUE_APP_OAUTH2_DOMAIN}
        },

        mounted() {
            this.getContexts();
            this.fullUser.role = this.config.userRole;
        },
        ...
        methods: {
            ...
            updateUser: function () {
                ...
                this.axios.post(process.env.VUE_APP_ROOT_API + "/users/", this.fullUser, {headers: {Authorization: localStorage.token}})
                    .then(() => {
                        this.$awn.success(this.$t("admin.users.modal.create.toaster.created"));
                        this.$emit("update");
                        this.show=false;
                    })
                    .catch(() => {
                        this.$awn.alert(this.$t("admin.users.modal.create.toaster.createError"));
                    });
            },
        }
    }
...
<template>
    <b-modal id="bv-modal-example" hide-footer v-model="show">
        ...
        <template v-slot:modal-title>{{$t("admin.users.modal.create.title")}}</template>

        <b-form-radio-group :label="$t('admin.users.modal.common.domainLabel')" v-model="fullUser.domain">
            <b-form-radio value="local">{{$t("admin.users.modal.common.localAccount")}}</b-form-radio>
            <b-form-radio v-if="oauth2.length!==0" :value="oauth2">{{$t("admin.users.modal.common.oauthAccount", [oauth2])}}</b-form-radio>
        </b-form-radio-group>
        <br />
        ...
        <b-form-group id="input-group-1" :label="$t('admin.users.modal.common.usernameLabel')" label-for="input-1">
            <b-form-input id="input-1" v-model="fullUser.username" type="text" required/>
        </b-form-group>
        ...
        <div class="text-right">
            ...
            <b-button class="mt-3 btn-warning" @click="updateUser()">{{$t("admin.users.modal.create.createButton")}}</b-button>
        </div>
    </b-modal>
</template>