    ...
    export default {
        ...
        methods: {
            updateAvailableContexts() {
                this.availableContexts = this.$props.contexts.filter(e => e.level < this.maxLevel-1 && e.id !== this.$props.context.id && !e.parentList.includes(this.$props.context.id) );
                this.$forceUpdate();
            },

            display(){
                this.form = {
                    name: this.$props.context.baseName,
                    description: this.$props.context.description,
                };

                this.form.parentContext = (this.$props.context.parentContext) ? this.$props.context.parentContext.id : null;
                this.show=true;
            ...
            createContext: function () {
                ...
                this.axios.put(process.env.VUE_APP_ROOT_API+ "/contexts/" + this.$props.context.id, this.form, {headers: {Authorization: localStorage.token}})
                    .then(() => {
                        this.$awn.success(this.$t("admin.contexts.modal.edit.toaster.updated"));
                        this.$emit('updated');
                        this.show=false;
                    })
                    .catch(() => {
                        this.$awn.alert(this.$t("admin.contexts.modal.edit.toaster.updateError"));
                    })
            },
...
<template>
    <b-modal id="bv-modal-example" hide-footer v-model="show">
        <template v-slot:modal-title>{{$t("admin.contexts.modal.edit.title")}}</template>
        <br/>
        ...
        <div class="text-right">
            ...
            <b-button class="mt-3 btn-success" @click="createContext()" :disabled="isDisabled">{{$t("admin.contexts.modal.edit.updateButton")}}</b-button>
        </div>
    </b-modal>
</template>